...
public class RWOpeningDirector : MonoBehaviour {
    ...
    public GameObject currentStageWitchCharacter;
    ...
    void Start()
    {
        if (openingDirection == true)
        {
            ...
            SendMessage("OpeningSkipInitializeDelivery");
        }
    }
    ...
    IEnumerator OpeningTake1Acting()
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            {
                currentStageWitchCharacter = Instantiate(witchCharacter, new Vector3(10.0f, 2.5f, 27.5f), target.transform.rotation) as GameObject;    //스테이지 마녀 생성
                currentStageWitchCharacter.GetComponent<Rigidbody2D>().gravityScale = 0;
                currentStageWitchCharacter.SendMessage("AllSpriteRendererSortingLayerStage");
                currentStageWitchCharacter.transform.localScale = new Vector3(0.3f, 0.3f, 0.3f);

                currentStageWitchCharacter.GetComponent<RWWitchHandler>().currentWitchState = RWWitchHandler.WitchState.Run;
                currentStageWitchCharacter.GetComponent<RWWitchHandler>().CheckWitchState();
                currentStageWitchCharacter.GetComponent<Puppet2D_GlobalControl>().flip = true;
                currentStageWitchCharacter.GetComponent<RWWitchHandler>().isMiniWitch = true;
            }
    ...
    IEnumerator OpeningTake2Acting()
    {
        ...
        explosionEffect.transform.FindChild("tonado").SendMessage("RealEffectPlay");
    ...
    public void OpeningTake4Acting()
    {
        if (openingDirection == true)
        {
            ...
            currentStageWitchCharacter.SendMessage("FlyingInitialize");

            //currentWitchCharacter.transform.localScale = new Vector3(0.8f, 0.8f, 0.8f);
            SendMessage("RWModeUISetting");
            SendMessage("OpeningSkipPanelOffDelivery");

            SendMessage("GPGSInitialize");
        }
    }
    ...
    public void FlyingTheWitchDelivery()
    {
        ...
        currentStageWitchCharacter.SendMessage("FlyingTheWitch");
    }
    ...
    public void FlyingTheWitchOffDelivery()
    {
        if (currentWitchCharacter != null)
        {
            ...
            currentStageWitchCharacter.SendMessage("FlyingTheWitchOff");
        }
