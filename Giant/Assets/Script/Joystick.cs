...
namespace UnityEngine.UI
{
    ...
    public class Joystick : UIBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        {
            _isDragging = false;
            JoystickGraphic.SendMessage("CurrentStickNeutral");
            playerCharacter.SendMessage("PlayerMove", new Vector2 (0,0));
        }
        ...
        void Update()
        {
            if (_isDragging)
                if (!dontCallEvent)
                    ...
                    {
                        OnValueChange.Invoke(JoystickAxis);
                        playerCharacter.SendMessage("PlayerMove", JoystickAxis);
                    }
        }
        ...
        {
            if (_joystickGraphic)
                _joystickGraphic.localPosition = _axis * Mathf.Max(rectTransform.sizeDelta.x, rectTransform.sizeDelta.y) * .3f;     //조이패드 벗어나는 범위
        }

        [System.Serializable]
        public class JoystickMoveEvent : UnityEvent<Vector2> { }

        public void CompulsionDragLeftUp()
        {
            SetAxis(new Vector2(-1.0f, 1.0f));
            _isDragging = true;
        }

        public void CompulsionDragCenterUp()
        {
            SetAxis(new Vector2(0, 1.0f));
            _isDragging = true;
        }

        public void CompulsionDragRightUp()
        {
            SetAxis(new Vector2(1.0f, 1.0f));
            _isDragging = true;
        }

        public void CompulsionDragLeftCenter()
        {
            SetAxis(new Vector2(-1.0f, 0));
            _isDragging = true;
        }

        public void CompulsionDragRightCenter()
        {
            SetAxis(new Vector2(1.0f, 0));
            _isDragging = true;
        }

        public void CompulsionDragLeftDown()
        {
            SetAxis(new Vector2(-1.0f, -1.0f));
            _isDragging = true;
        }

        public void CompulsionDragCenterDown()
        {
            SetAxis(new Vector2(0, -1.0f));
            _isDragging = true;
        }

        public void CompulsionDragRightDown()
        {
            SetAxis(new Vector2(1.0f, -1.0f));
            _isDragging = true;
        }

        public void CompulsionEnd()
        {
            _isDragging = false;
            JoystickGraphic.SendMessage("CurrentStickNeutral");
            playerCharacter.SendMessage("PlayerMove", new Vector2(0, 0));
